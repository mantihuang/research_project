---
title: "Structural Vector Autoregressions (SVARs) Application: the effects of unanticipated government spending shocks on Australian economy"
...
Most economies have experienced significant shocks due to the pandemic of the COVID-19,resulting in a marked increase in the government net debt-to-GDP ratios. For instance, the Australian's general government net debt-to-GDP ratio surged from 24.5\% in 2019 to 38.1\% in 2021, according to [@abs2021government], and the United States's ratio rises from 106.1\% to 121.1\% for the same periods by [@fed2023federal]. 

There has been considerable researches on the consequences of government spending shocks on output, consumption, and other crucial macroeconomic factors. Economic theory suggests that positive shocks result in increased output and consumption. However, there has been less emphasis on the economy's external sector, including real exchange rates, imports, and exports. By employing the Structural Vector Autoregressions (SVARs) model to identify government spending shocks and investigating their connection with these variables, policymakers can enhance the formulation and execution of fiscal and monetary policies in response to unanticipated government spending shocks.

The research project aims to model the effect of government spending shocks on output, real exchange rate and consumption; and answering the question how the unanticipated government spending shocks influence output, real exchange rate and consumption in the short run and long run. 
...
adf_summary <- map_df(df_data_frame , function(x) {
  adf <- adf.test(x, k = 4)
  tibble(p_value = round(adf$p.value,2))
}, .id = "Variable")
...
$$y_t=\begin{pmatrix}  
govspend_t &= \text{government spending}
...
2. Draw $\kappa_A^{(s)} \sim p(\kappa_A |Y,X,A^{(s)},\Sigma^{(s)})$.
...
Skappacheck
```
As can be seen from the results above, the hyperparamter extension also works.
...
Sign_restritions = SignRestrictions(sign.restrictions = c(1,1,0,0,0,1,1,1), results)
 # Narrative_Restriction = NarrativeSignRestrictions(sign.restrictions = c(1,1,1,-1,1,-1,1,1), results,c(16,50),Y,X)
...
S = 800
# Narrative_Restriction = NarrativeSignRestrictions(sign.restrictions = c(1,1,0,0,0,1,1,1), Posterior_kappa,c(16,50,80),Y,X)
# Sign_restritions = SignRestrictions(sign.restrictions = c(1,1,0,-1,1,1,1,1), results)
...
# 
# ############################### From the porsterior distribution#########
# A.posterior = Posterior_kappa$A.posterior
# # B.posterior = results$B.posterior
...
1. Initially, I observed explosive Impulse Response Functions (IRF). However, I found that applying a logarithmic transformation to every variable greatly alleviates this problem.

2. I fixed the problem of A posterior by putting the transformation from B0 and B1 to A inside the sign-restriction function.
